...
- [ ] Install and configure chart library (AC: 1)
  - [ ] Install recharts: `npm install recharts`
  - [ ] Install required types: `npm install -D @types/recharts`
  - [ ] Verify installation and add to build

- [ ] Create `usePriceHistory.ts` hook for fetching historical data (AC: 1, 2, 7, 8)
  - [ ] Implement SWR hook to fetch from `/coins/{id}/market_chart` endpoint
  - [ ] Add query parameters for `vs_currency=usd` and `days={selected}`
  - [ ] Handle loading and error states
  - [ ] Add proper TypeScript types for price history data
  - [ ] Map API response to chart-friendly format

- [ ] Create `PriceHistoryChart.tsx` component (AC: 1, 2, 3, 9)
  - [ ] Build chart using Recharts Line component
  - [ ] Format x-axis with appropriate date/time labels based on range
  - [ ] Format y-axis with currency formatting
  - [ ] Add responsive container for mobile compatibility
  - [ ] Style chart to match existing dark/light theme

- [ ] Create `TimeRangeSelector.tsx` component (AC: 2, 3)
  - [ ] Use shadcn/ui ToggleGroup for time range options
  - [ ] Implement options: 24h, 7d, 30d, 90d, 1y
  - [ ] Style active state to match existing UI patterns
  - [ ] Handle range selection and emit change events
